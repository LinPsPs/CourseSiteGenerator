...
public class TimeSlot {
    ...
    public void addTA(DayOfWeek dow, TeachingAssistantPrototype ta) {
        ...
        TAType currentType;
        if (CourseSiteController.isAllMode) {
            currentType = TAType.All;
        }
        else {
            currentType = TAType.valueOf(ta.getType());
        }
    ...
    public boolean isOfficeHour(String initStartHour, String initEndHour) {
        String startHour = startTime.getValue();
        String endHour = endTime.getValue();
        initEndHour = initEndHour.substring(0,initEndHour.length()-2)+" "+initEndHour.substring(initEndHour.length()-2, initEndHour.length());
        initStartHour = initStartHour.substring(0,initStartHour.length()-2)+" "+initStartHour.substring(initStartHour.length()-2, initStartHour.length());
        startHour = startHour.substring(0,startHour.length()-2)+" "+startHour.substring(startHour.length()-2, startHour.length());
        endHour = endHour.substring(0,endHour.length()-2)+" "+endHour.substring(endHour.length()-2, endHour.length());
        SimpleDateFormat sdf = new SimpleDateFormat("h:mm a");
        try{
            Date startHourDate = sdf.parse(startHour);
            Date endHourDate = sdf.parse(endHour);
            Date initStartHourDate = sdf.parse(initStartHour);
            Date initEndHourDate = sdf.parse(initEndHour);
            
            if((initStartHourDate.compareTo(startHourDate) <= 0) &&
                    ((endHourDate.compareTo(initEndHourDate) <= 0))) {
                return true;
            }
        }
        catch(Exception e) {
            System.out.println("Something wrong in TimeSlot - isOfficeHour");
            return false;
        }
        return false;
    }
